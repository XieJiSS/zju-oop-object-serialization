...
namespace serializer {
  ...
  namespace {
    ...
    template <size_t i = 0, typename Func, typename... Tp>
        ...
        i<sizeof...(Tp), void> foreach_in_tuple(const std::tuple<Tp...> &t, Func f) {
      f(std::get<i>(t), i);
      foreach_in_tuple<i + 1, Func, Tp...>(t, f);
    }
  } // namespace
  ...
  namespace {
    ...
    template <typename T>
    struct P<T, std::void_t<typename T::first_type, // first_type
  ...
  namespace {
    ...
    struct A<Container<T, Alloc>> {
      ...
      static constexpr bool v =
          std::is_same_v<prob_t, std::list<T, Alloc>> || std::is_same_v<prob_t, std::vector<T, Alloc>>;
    };
  } // namespace
  template <typename T>
  ...
  namespace {
    ...
    struct C<Container<T, Alloc>> {
      ...
      using prob_t = Container<T, Alloc>;
      static constexpr bool v = is_pair_v<prob_t> || is_array_container_v<prob_t> ||
                                is_map_container_v<prob_t> || is_set_container_v<prob_t>;
    };
  } // namespace
  ...
  template <typename T>
  constexpr auto is_string_cstring_v =
      std::is_same_v<remove_cv_t<T>, string> || is_cstring_v<T>; // ditto, no remove_cv_t.

  template <typename T>
  constexpr auto is_supported_v =
      is_supported_container_v<remove_cv_t<T>> || is_string_cstring_v<remove_cv_t<T>> ||
      std::is_arithmetic_v<T>; // is_arithmetic_v has remove_cv_t inside.

  template <typename T>
  constexpr auto is_supported_literal_v =
      is_supported_v<remove_cv_t<T>> && !is_supported_container_v<remove_cv_t<T>>;
